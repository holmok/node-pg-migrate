...
const parseColumns = (tableName, columns, mOptions) => {
  ...
  let columnsWithOptions = lodash_1.default.mapValues(columns, (column) =>
    utils_1.applyType(column, extendingTypeShorthands),
  )
  const primaryColumns = lodash_1.default
  ...
  if (multiplePrimaryColumns) {
    columnsWithOptions = lodash_1.default.mapValues(columnsWithOptions, (options) =>
      Object.assign(Object.assign({}, options), { primaryKey: false }),
    )
  }
  const comments = lodash_1.default
    .chain(columnsWithOptions)
    .map(
      (options, columnName) =>
...
const parseLike = (like, literal) => {
  const formatOptions = (name, options) =>
    (lodash_1.default.isArray(options) ? options : [options])
...
function dropColumns(mOptions) {
  const _drop = (tableName, columns, options = {}) => {
    ...
    const columnsStr = utils_1.formatLines(
